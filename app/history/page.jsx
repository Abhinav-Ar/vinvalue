...
export default function ActivityPage() {
  ...
  return <div className="min-h-screen"><SiteHeader /><main className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-14">
    ...
    {!loading && items.length > 0 && <div className="mt-10 overflow-hidden rounded-3xl border border-border bg-card">{items.map((item, index) => <article key={item.id} className={`grid gap-5 p-5 sm:grid-cols-[1fr_auto] sm:items-center sm:p-6 ${index ? "border-t border-border" : ""}`}><div className="min-w-0"><div className="flex items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-muted"><FileText className="h-4 w-4" /></span><div className="min-w-0"><h2 className="truncate font-semibold">{item.year} {item.make} {item.model}{item.trim ? ` ${item.trim}` : ""}</h2><p className="mt-1 text-xs text-muted-foreground">{Number(item.mileage).toLocaleString()} mi · {item.condition} · {date(item.created_at)}</p></div></div></div><div className="flex items-center justify-between gap-6 sm:justify-end"><div><p className="data-label">Instant sale</p><p className="mt-1 text-xl font-semibold">{money(item.trade_in)}</p></div>{item.profile_encoded && <Button asChild variant="outline" className="rounded-xl"><Link href={`/profile?d=${item.profile_encoded}`} target="_blank">Report <ExternalLink /></Link></Button>}</div></article>)}</div>}
  </main></div>;
}
